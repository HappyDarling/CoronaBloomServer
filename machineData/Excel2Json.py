...
data = {}
data['당황'] = []
for w in range(len(df_conv[df_conv['감정_대분류'].isin(['당황'])])):
    if not (pd.isnull(df_conv[df_conv['감정_대분류'].isin(['당황'])].iloc[w, 7])):
        data['당황'].append(df_conv[df_conv['감정_대분류'].isin(['당황'])].iloc[w, 7]);
